...
public class ToStringSteps {
    ...
    public ToStringSteps(PlanningAppHolder planningAppHolder, ProjectHolder projectHolder,
    ...
    public void theActivityStartsInWeekOf(Integer startWeek, Integer startYear)
        ...
        planningAppHolder.getPlanningApp().editStartDateOfActivity(startDate, project.getProjectNumber(),
                activity.getName(), projectHolder.getProject().getProjectLeader().getInitials());
    }
    ...
    public void theActivityEndsInWeekOf(Integer endWeek, Integer endYear)
        ...
        Project project = projectHolder.getProject();
        Activity activity = activityHolder.getActivity();
        planningAppHolder.getPlanningApp().editEndDateOfActivity(endDate, project.getProjectNumber(),
                activity.getName(), projectHolder.getProject().getProjectLeader().getInitials());

    }

    @Given("employee with initials {string} is assigned to the activity")
    public void employeeWithInitialsIsAssignedToTheActivity(String initials)
            throws NotProjectLeaderException, ActivityNotFoundException {
        Project project = projectHolder.getProject();
        ...
        // If this step is used by other features than to-string this might prevent a proper setup since is overrides
        // projectleader that might be set by anohter given
    ...
    public void iMatchTheProjectThatId() {
        // This looks odd, but its still a sanity check. It ensures that the project can be found with the match
        // function.
    ...
    public void iGetTheProjectStringWithThatIDInTheEnd(String wantedString) {
        // This string test is needed since the project id is unknown. It depends on when its created what the id would
        // be. Any test must be independent of when someone run it.

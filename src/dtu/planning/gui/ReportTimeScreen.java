...
public class ReportTimeScreen {
    /*
     * flow idea: Select which employee to report time for Show all the activities that the employee is assigned and
     * which project they belong to Select the activity to report time on Give amount of time with units and date (the
     * date could be set to today as standard) Finally, the time can be reported. Afterwards, a success message will be
     * shown
     */

    /*
     * Implementation: There will be one master panel that contains three other panels. Only one of these panels will be
     * shown at a time The first panel will say "Step 1" and here you will be able to search for and select an employee.
     * At the bottom of this panel there will be a button to take you to the next panel. You must select an employee to
     * move forward The second panel will say "Step 2" and all the activities that the employee is assigned to will be
     * shown. At the bottom of the second panel there will be a previous and next button. You must select an Activity to
     * move forward. In the third panel it will be possible to give the amount of time in hours (float) At the bottom of
     * the third panel there will be a previous button and a report time button
     */
    ...
    private void initialize() {
        ...
        panelReportTime.setBorder(BorderFactory.createTitledBorder("Report Time"));
        ...
        b = new StringBuffer();
        b.append("<html><h2>Previous</h2></html>");
        ...
        hoursComboBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                float time = Float.parseFloat(minutesComboBox.getSelectedItem().toString()) / 60
                        + Float.parseFloat(hoursComboBox.getSelectedItem().toString());
        ...
        b = new StringBuffer();
        b.append("<html><h2>Report Time</h2></html>");
        ...
        b = new StringBuffer();
        b.append("<html><h2>New Time Report</h2></html>");
        ...
        btnPrevious = new JButton();
        b = new StringBuffer();
        b.append("<html><h2>Great!</h2></html>");
        btnPrevious.setText(b.toString());
    ...
    private void setSuccessMessage() {
        ...
        b.append(dayComboBox.getSelectedItem().toString() + "/" + monthComboBox.getSelectedItem().toString() + "/"
    ...
    private void findRelevantActivities() {
        relevantActivities.clear();
        SimpleEntry<List<Activity>, List<Integer>> foundActivities = planningApp
                .getAllRelevantActivitiesForEmployee(employee.getInitials());
        foundActivities.getKey().forEach((a) -> {
    ...
    protected void searchEmployees() {
        ...
        searchResults.clear();
        try {
            planningApp.searchForEmployeesByName(searchField.getText()).forEach((m) -> {
                searchResults.addElement(m);
            });
    ...
    private void reportTime() {
        GregorianCalendar date = new GregorianCalendar(Integer.parseInt(yearComboBox.getSelectedItem().toString()),
                Integer.parseInt(monthComboBox.getSelectedItem().toString()) - 1,

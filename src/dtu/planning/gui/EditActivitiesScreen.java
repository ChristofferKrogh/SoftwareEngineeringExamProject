...
public class EditActivitiesScreen {
    ...
    private void initialize() {
        ...
        panelEditActivities.setBorder(BorderFactory.createTitledBorder("Edit Activities"));
        ...
        btnAdd = new JButton();
        b = new StringBuffer();
        b.append("<html><h2>Add-activity</h2></html>");
        ...
        b = new StringBuffer();
        b.append("<html><h2>Remove</h2></html>");
    ...
    private void initializeSearchResults() {
        project.getActivities().forEach(a -> {
            searchResults.addElement(a);
        });
    }

    private void initializeSearchResultsEmployee() {
        try {
            activity.getAssignedEmployees().forEach((m) -> {
                searchResultEmployee.addElement(m);
            });
        } catch (Exception e) {
    ...
    public void setProject(Project project) {
        ...
        if (project.getProjectLeader() != null) {
            ...
            b = new StringBuffer();
            b.append("<html><h2>There has not been selected a project leader for</h2><h1>" + project.getName()
                    + "</h1></html>");
            lblCheckLeader.setText(b.toString());
    ...
    public void searchActivities() {
        searchResults.clear();
        try {
            planningApp.searchForActivitiesByName(project.getProjectNumber(), searchField.getText()).forEach((m) -> {
    ...
    public void editActivity() {
        ...
        if (!amountOfHours.getText().equals("")) {
            ...
            try {
                planningApp.editExpectedAmountOfHoursForActivity(hours, project.getProjectNumber(), name,
                        project.getProjectLeader().getInitials());
    ...
    private void removeEmployee() {
        try {
            planningApp.removeEmployeeFromActivity(project.getProjectNumber(), activity.getName(),
                    listSearchResultEmployee.getSelectedValue().getInitials(),
                    project.getProjectLeader().getInitials());
    ...
    private void searchAllEmployees() {
        ...
        try {
            planningApp.searchForEmployeesByName(searchFieldAddEmployee.getText()).forEach((m) -> {
                searchResultsAddEmployee.addElement(m);
            });
        } catch (OperationNotAllowedException e) {
            setConsoleMessage(e.getMessage());
        }
    ...
    private void addEmployee() {
        ...
        try {
            planningApp.assignEmployee(project.getProjectNumber(), activity.getName(),
                    project.getProjectLeader().getInitials(), initials);
        } catch (Exception e) {
            setConsoleMessage(e.getMessage());
        }
    }

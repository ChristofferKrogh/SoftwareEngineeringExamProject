...
public class PlanningApp {
    ...
    public void editStartDateOfProject(GregorianCalendar startDate, int projectNumber)
    ...
    public void editStartWeekOfRegular(GregorianCalendar startWeek, String regularActivityName)
            throws OperationNotAllowedException {
        searchForRegActivity(regularActivityName).setStartWeek(startWeek);
    }

    public void editEndWeekOfRegular(GregorianCalendar endWeek, String regularActivityName)
            throws OperationNotAllowedException {
    ...
    public List<Activity> searchForActivitiesByName(int projectNumber, String searchText)
            throws OperationNotAllowedException {
        List<Activity> searchResults = new ArrayList<>();
    ...
    // Method 1 in report
    public void assignEmployee(int projectNumber, String activityName, String projectLeaderInitials,
    ...
    private void checkEmployeeExist(Employee employee) throws OperationNotAllowedException {
        Optional<Employee> r = employees.stream().filter(b -> b.getInitials().equals(employee.getInitials()))
                .findFirst();
        if (!r.isPresent()) {
            throw new OperationNotAllowedException("The employee does not exist");
        }
    ...
    // Method 2 in report
    public Activity addActivity(int projectNumber, String activityName, GregorianCalendar startWeek,
            GregorianCalendar endWeek, int expectedAmountOfHours, String actorInitials)
    ...
    public void editStartDateOfActivity(GregorianCalendar startDate, int projectNumber, String name,
            String projectLeaderInitials)
            throws ActivityNotFoundException, OperationNotAllowedException, NotProjectLeaderException {
        // Find project from id
        Project project = this.searchForProject(projectNumber);
    ...
    public void editEndDateOfActivity(GregorianCalendar endDate, int projectNumber, String name,
            String projectLeaderInitials)
            throws ActivityNotFoundException, OperationNotAllowedException, NotProjectLeaderException {
        // Find project from id
        Project project = this.searchForProject(projectNumber);
    ...
    // Method 3 in report
    public void editExpectedAmountOfHoursForActivity(float expectedAmountOfHours, int projectNumber,
            String activityName, String projectLeaderInitials)
            throws ActivityNotFoundException, OperationNotAllowedException, NotProjectLeaderException {
        // Find project from id
        Project project = this.searchForProject(projectNumber);
        ...
        assert Float.compare(activity.getExpectedAmountOfHours(),
                expectedAmountOfHours) == 0 : "Postcondition violated";
    }

    public void removeEmployeeFromActivity(int projectNumber, String activityName, String oldEmployeeInitials,
            String projectLeaderInitials)
            throws OperationNotAllowedException, NotProjectLeaderException, ActivityNotFoundException {
        // Find project from id
        Project project = this.searchForProject(projectNumber);
        ...
        if (!project.getActivityByName(activityName).getAssignedEmployees().stream().map(e -> e.getInitials())
                .anyMatch(i -> i.equals(oldEmployeeInitials))) {
    ...
    public Activity searchForActivity(int projectNumber, String activityName)
    ...
    // Method 4 in report
    public void registerTime(int projectNumber, String activityName, TimeRegistration timeRegistration)
    ...
    public void correctTimeReport(int projectNumber, String activityName, TimeRegistration timeRegistration,
            int amountOfTime) throws OperationNotAllowedException, ActivityNotFoundException {
        // Find project from id
        Project project = this.searchForProject(projectNumber);
    ...
    public Report generateReport(int projectNumber, Employee projectLeader)
            throws NotProjectLeaderException, OperationNotAllowedException {
        // Find project from id
        Project project = this.searchForProject(projectNumber);
    ...
    public int getDailyUsedTime(String initials, GregorianCalendar date)
    ...
    public SimpleEntry<List<Activity>, List<Integer>> getAllRelevantActivitiesForEmployee(String employeeInitials) {
        ...
        for (Project p : projects) {
            for (Activity a : p.getActivities()) {
                if (a.getAssignedEmployees().stream().map(e -> e.getInitials())
                        .anyMatch(i -> i.equals(employeeInitials))) {

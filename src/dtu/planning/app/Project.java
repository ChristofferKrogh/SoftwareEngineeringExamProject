...
public class Project {
    ...
    public void addActivity(Activity activity, String actorInitials)
            throws NotProjectLeaderException, OperationNotAllowedException {
        // Check project leader
        this.isProjectLeader(actorInitials);

        // Check for duplicate
        Optional<Activity> r = activities.stream().filter(b -> b.getName().equals(activity.getName())).findFirst();
        if (r.isPresent()) {
    ...
    public void assignEmployee(String activityName, Employee projectLeader, Employee employee)
    ...
    public Activity getActivityByName(String activityName) throws ActivityNotFoundException {
        ...
        Optional<Activity> r = activities.stream().filter(b -> b.getName().equals(activityName)).findFirst();
        if (!r.isPresent()) {
    ...
    private void isProjectLeader(String projectLeaderInitials) throws NotProjectLeaderException {
        if (this.projectLeader == null || projectLeaderInitials == null
                || !this.projectLeader.getInitials().equals(projectLeaderInitials)) {
